...
class Decoder(tf.keras.Model):
    ...
    def __init__(self, dropout_probs=hyp.dropout_rate):
        ...
        self.conv1 = tf.keras.layers.SeparableConv2D(filters=hyp.base_num_covs*5, kernel_size=3, strides=1, padding='same', activation="relu", use_bias=False)
        ...
        self.upsample5 = tf.keras.layers.Conv2DTranspose(hyp.base_num_covs*1, 3, strides=2,padding='same')
...
class LEO(tf.keras.Model):
    ...
    def __init__(self, mode="meta_train"):
        ...
        self.seg_weight = tf.Variable(self.init((3, 3, hyp.base_num_covs*1 + 3, 2)))
