...
def test_saved_model(dataset, encoder, decoder, chosen_encoder, input_imgs, img_dims):
    oracle = forward_model(encoder, decoder, input_imgs)
    retrieved_encoder = chosen_encoder(img_dims)
    retrieved_encoder.load_weights(f"./data/{dataset}/embedding_ckpt/encoder_weights")
    retrieved_decoder = Decoder()
    retrieved_decoder.load_weights(f"./data/{dataset}/embedding_ckpt/decoder_weights")
...
def test_saved_embeddings(dataset):
    emb_path_root = os.path.join(os.path.dirname(__file__), "data", f"{dataset}", "embeddings")
    data_path_root = os.path.join(os.path.dirname(__file__), "data", f"{dataset}")
...
def save_embeddings(encoder, decoder, data_type, dataset, **kwargs):
    path_root = os.path.join(os.path.dirname(__file__), "data", f"{dataset}", "embeddings")
...
class SampleOneClass(Dataset):
    ...
    def __init__(self, class_name, train_or_val,**kwargs):
        ...
        self.img_paths = [i[0] for i in self.img_datasets.imgs if class_name == i[0].split(os.sep)[-2]]
        self.mask_paths = [i[0] for i in self.mask_datasets.imgs if class_name == i[0].split(os.sep)[-2]]
...
def train_model(encoder, decoder, epochs, freq, show_stats=False,**model_kwargs):
    ...
    encoder.save_weights(f"./data/{dataset}/embedding_ckpt/encoder_weights")
    decoder.save_weights(f"./data/{dataset}/embedding_ckpt/decoder_weights")
    return training_stats, iou_per_class_list, decoder, batch_imgs

def main(dataset, **train_kwargs):
    """
    Main function
    ...
    epochs, freq, bs = train_kwargs.get("epochs", 1), train_kwargs.get("freq", 1), train_kwargs.get("bs", 1)
    ...
    grouped_by_classes_root = os.path.join(os.path.dirname(__file__), "data", f"{dataset}")
    ...
    train_stats_save_path_root = os.path.join(os.path.dirname(__file__), "data", f"{dataset}", "emb_train_stats")
    os.makedirs(train_stats_save_path_root, exist_ok=True)

    test_saved_model(dataset, encoder, decoder, chosen_encoder, batch_imgs, img_dims)
    ...
    if generate_embeddings:
        save_embeddings(encoder, decoder, "train", dataset, **model_kwargs)
        save_embeddings(encoder, decoder,"val", dataset, **model_kwargs)
    test_saved_embeddings(dataset)
